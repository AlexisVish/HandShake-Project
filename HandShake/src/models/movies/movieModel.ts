...
export const createMovie = (movie: Movie, callback: (err: Error | null, movieId?: number) => void) => {
    const stmt = db.prepare('INSERT INTO movies (title, genre, year, image_url) VALUES (?, ?, ?, ?)');
    stmt.run(movie.title, movie.genre, movie.year, movie.image_url, function (err) {
